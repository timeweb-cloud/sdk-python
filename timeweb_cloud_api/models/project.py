...
class Project(BaseModel):
    ...
    Проект
    """
    id: Optional[Any] = Field(..., description="ID для каждого проекта. Автоматически генерируется при создании.")
    account_id: Optional[Any] = Field(..., description="ID пользователя.")
    avatar_id: Optional[Any] = Field(..., description="ID аватара пользователя. Описание методов работы с аватарами появится позднее.")
    description: Optional[Any] = Field(..., description="Описание проекта.")
    name: Optional[Any] = Field(..., description="Удобочитаемое имя проекта.")
    is_default: Optional[Any] = Field(..., description="Это логическое значение, которое показывает, выбран ли проект по умолчанию для создания новых сущностей.")
    __properties = ["id", "account_id", "avatar_id", "description", "name", "is_default"]
