...
class RouterPreset(BaseModel):
    """
    RouterPreset
    """
    id: Optional[Any] = Field(..., description="ID тарифа")
    node_count: Optional[Any] = Field(..., description="Количество нод")
    ...
    ram: Optional[Any] = Field(..., description="Размер ОЗУ в ГБ")
    bandwidth: Optional[Any] = Field(..., description="Пропускная способность")
    cost: Optional[Any] = Field(..., description="Цена в рублях")
    location: Optional[Any] = Field(..., description="Локация")
    __properties = ["id", "node_count", "cpu", "cpu_frequency", "ram", "bandwidth", "cost", "location"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RouterPreset:
    ...
    def to_dict(self):
        ...
        # and __fields_set__ contains the field
        if self.node_count is None and "node_count" in self.__fields_set__:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RouterPreset:
        """Create an instance of RouterPreset from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RouterPreset.parse_obj(obj)

        _obj = RouterPreset.parse_obj({
            "id": obj.get("id"),
