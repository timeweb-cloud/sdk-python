...
class CreateApp(BaseModel):
    ...
    CreateApp
    """
    provider_id: Optional[Any] = Field(..., description="Уникальный идентификатор провайдера.")
    type: Optional[Any] = Field(..., description="Тип приложения.")
    repository_id: Optional[Any] = Field(..., description="Уникальный идентификатор репозитория.")
    build_cmd: Optional[Any] = Field(..., description="Команда сборки приложения.")
    envs: Optional[Any] = Field(None, description="Переменные окружения приложения. Объект с ключами и значениями типа string.")
    branch_name: Optional[Any] = Field(..., description="Название ветки репозитория из которой необходимо собрать приложение.")
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('frontend', 'backend'):
            raise ValueError("must be one of enum values ('frontend', 'backend')")
        return value
    ...
    def from_dict(cls, obj: dict) -> CreateApp:
        ...
        if not isinstance(obj, dict):
            return CreateApp.parse_obj(obj)

        _obj = CreateApp.parse_obj({
