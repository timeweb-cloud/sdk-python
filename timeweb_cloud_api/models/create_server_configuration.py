...
class CreateServerConfiguration(BaseModel):
    """
    Параметры конфигурации сервера. Нельзя передавать вместе с `preset_id`.
    """
    configurator_id: Optional[Any] = Field(..., description="ID конфигуратора сервера.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateServerConfiguration:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateServerConfiguration:
        """Create an instance of CreateServerConfiguration from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateServerConfiguration.parse_obj(obj)

        _obj = CreateServerConfiguration.parse_obj({
