...
class AddonsConfigResponse(BaseModel):
    ...
    k8s_addons: Optional[Any] = Field(..., description="Массив конфигураций дополнений k8s")
    __properties = ["response_id", "meta", "k8s_addons"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AddonsConfigResponse:
    ...
    def to_dict(self):
        ...
        # set to None if k8s_addons (nullable) is None
        # and __fields_set__ contains the field
        if self.k8s_addons is None and "k8s_addons" in self.__fields_set__:
            _dict['k8s_addons'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> AddonsConfigResponse:
        """Create an instance of AddonsConfigResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AddonsConfigResponse.parse_obj(obj)

        _obj = AddonsConfigResponse.parse_obj({
            "response_id": obj.get("response_id"),
            "meta": SchemasMeta.from_dict(obj.get("meta")) if obj.get("meta") is not None else None,
            "k8s_addons": obj.get("k8s_addons")
        })
        return _obj
