...
class Db(BaseModel):
    ...
    local_ip: Optional[Any] = Field(..., description="IP-адрес локального сетевого интерфейса IPv4.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Db:
    ...
    def to_dict(self):
        ...
        # set to None if is_only_local_ip_access (nullable) is None
        # and __fields_set__ contains the field
        if self.is_only_local_ip_access is None and "is_only_local_ip_access" in self.__fields_set__:
            _dict['is_only_local_ip_access'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> Db:
        """Create an instance of Db from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Db.parse_obj(obj)
