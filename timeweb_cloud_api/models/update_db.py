...
class UpdateDb(BaseModel):
    ...
    """
    password: Optional[Any] = Field(None, description="Пароль для подключения к базе данных.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateDb:
    ...
    def to_dict(self):
        ...
        # set to None if is_external_ip (nullable) is None
        # and __fields_set__ contains the field
        if self.is_external_ip is None and "is_external_ip" in self.__fields_set__:
            _dict['is_external_ip'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> UpdateDb:
        """Create an instance of UpdateDb from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateDb.parse_obj(obj)

        _obj = UpdateDb.parse_obj({
            ...
            "is_external_ip": obj.get("is_external_ip")
        })
        return _obj
