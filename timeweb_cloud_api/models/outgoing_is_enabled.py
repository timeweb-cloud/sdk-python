...
class OutgoingIsEnabled(BaseModel):
    """
    OutgoingIsEnabled
    """
    is_enabled: Optional[Any] = Field(..., description="Включена ли пересылка исходящих писем")
    outgoing_email: Optional[Any] = Field(..., description="Адрес для пересылки исходящих писем. \\  Если передан параметр `is_enabled`: `false`, то значение передавать нельзя")
    __properties = ["is_enabled", "outgoing_email"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OutgoingIsEnabled:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OutgoingIsEnabled:
        """Create an instance of OutgoingIsEnabled from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OutgoingIsEnabled.parse_obj(obj)

        _obj = OutgoingIsEnabled.parse_obj({
