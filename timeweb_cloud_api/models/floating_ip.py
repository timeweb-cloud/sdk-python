...
class FloatingIp(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FloatingIp:
    ...
    def to_dict(self):
        ...
        # and __fields_set__ contains the field
        if self.ptr is None and "ptr" in self.__fields_set__:
            _dict['ptr'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> FloatingIp:
        """Create an instance of FloatingIp from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FloatingIp.parse_obj(obj)

        _obj = FloatingIp.parse_obj({
            "id": obj.get("id"),
            "ip": obj.get("ip"),
