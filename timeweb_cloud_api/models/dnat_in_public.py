...
class DnatInPublic(BaseModel):
    """
    Публичный адрес
    """
    ip: Optional[Any] = Field(..., description="IP-адрес")
    port: Optional[Any] = Field(None, description="Порт или диапазон портов")
    __properties = ["ip", "port"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DnatInPublic:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DnatInPublic:
        """Create an instance of DnatInPublic from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DnatInPublic.parse_obj(obj)

        _obj = DnatInPublic.parse_obj({
            "ip": obj.get("ip"),
            "port": obj.get("port")
        })
        return _obj
