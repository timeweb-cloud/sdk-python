...
class UpdateServer(BaseModel):
    ...
    UpdateServer
    """
    configurator: Optional[UpdateServerConfigurator] = None
    os_id: Optional[Any] = Field(None, description="Уникальный идентификатор операционной системы, которая будет установлена на облачный сервер.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateServer:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateServer:
        """Create an instance of UpdateServer from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateServer.parse_obj(obj)

        _obj = UpdateServer.parse_obj({
            "configurator": UpdateServerConfigurator.from_dict(obj.get("configurator")) if obj.get("configurator") is not None else None,
            "os_id": obj.get("os_id"),
