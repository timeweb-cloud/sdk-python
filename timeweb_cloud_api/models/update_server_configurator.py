...
class UpdateServerConfigurator(BaseModel):
    """
    Параметры конфигурации сервера. Нельзя передавать вместе с `preset_id`.
    """
    configurator_id: Optional[Any] = Field(None, description="ID конфигуратора сервера.")
    disk: Optional[Any] = Field(None, description="Размер диска в МБ.")
    cpu: Optional[Any] = Field(None, description="Количество ядер процессора.")
    ram: Optional[Any] = Field(None, description="Размер ОЗУ сервера в МБ.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateServerConfigurator:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateServerConfigurator:
        """Create an instance of UpdateServerConfigurator from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateServerConfigurator.parse_obj(obj)

        _obj = UpdateServerConfigurator.parse_obj({
