...
class ClusterOut(BaseModel):
    ...
    ClusterOut
    """
    id: Optional[Any] = Field(..., description="ID кластера")
    name: Optional[Any] = Field(..., description="Название")
    created_at: Optional[Any] = Field(..., description="Дата и время создания кластера в формате ISO8601")
    status: Optional[Any] = Field(..., description="Статус")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClusterOut:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ClusterOut:
        """Create an instance of ClusterOut from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClusterOut.parse_obj(obj)

        _obj = ClusterOut.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
