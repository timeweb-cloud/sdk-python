...
class DomainRegister(BaseModel):
    ...
    person_id: Optional[Any] = Field(..., description="ID администратора, на которого регистрируется домен.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DomainRegister:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DomainRegister:
        """Create an instance of DomainRegister from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DomainRegister.parse_obj(obj)

        _obj = DomainRegister.parse_obj({
