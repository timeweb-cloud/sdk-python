...
class CreateCluster(BaseModel):
    ...
    def hash_type_validate_enum(cls, value):
        ...
        if value not in ('caching_sha2', 'mysql_native'):
            raise ValueError("must be one of enum values ('caching_sha2', 'mysql_native')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateCluster:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateCluster:
        """Create an instance of CreateCluster from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateCluster.parse_obj(obj)

        _obj = CreateCluster.parse_obj({
            "name": obj.get("name"),
            "type": obj.get("type"),
            "admin": CreateClusterAdmin.from_dict(obj.get("admin")) if obj.get("admin") is not None else None,
