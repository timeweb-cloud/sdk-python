...
class AddTokenPackage(BaseModel):
    """
    Данные для добавления дополнительного пакета токенов
    """
    count: Optional[Any] = Field(None, description="Количество пакетов для добавления (от 1 до 100)")
    __properties = ["count"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AddTokenPackage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AddTokenPackage:
        """Create an instance of AddTokenPackage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AddTokenPackage.parse_obj(obj)

        _obj = AddTokenPackage.parse_obj({
            "count": obj.get("count")
        })
        return _obj
