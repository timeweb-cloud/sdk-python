...
class DatabaseAdmin(BaseModel):
    """
    Пользователь базы данных
    """
    id: Optional[Any] = Field(..., description="Уникальный идентификатор для каждого экземпляра пользователя базы данных. Автоматически генерируется при создании.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DatabaseAdmin:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DatabaseAdmin:
        """Create an instance of DatabaseAdmin from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DatabaseAdmin.parse_obj(obj)

        _obj = DatabaseAdmin.parse_obj({
            "id": obj.get("id"),
