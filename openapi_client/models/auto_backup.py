...
class AutoBackup(BaseModel):
    ...
    AutoBackup
    """
    copy_count: Optional[Any] = Field(None, description="Количество копий для хранения. Минимальное количество `1`, максимальное `99`")
    creation_start_at: Optional[Any] = Field(None, description="Дата начала создания первого автобэкапа. Значение в формате `ISO8601`. Время не учитывается.")
    is_enabled: Optional[Any] = Field(..., description="Включено ли автобэкапирование")
    interval: Optional[Any] = Field(None, description="Периодичность создания автобэкапов")
    day_of_week: Optional[Any] = Field(None, description="День недели, в который будут создаваться автобэкапы. Работает только со значением `interval`: `week`. Доступные значение от `1 `до `7`.")
    __properties = ["copy_count", "creation_start_at", "is_enabled", "interval", "day_of_week"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AutoBackup:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AutoBackup:
        """Create an instance of AutoBackup from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AutoBackup.parse_obj(obj)

        _obj = AutoBackup.parse_obj({
