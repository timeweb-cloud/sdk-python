...
class DedicatedServerAdditionalService(BaseModel):
    """
    Дополнительная услуга для выделенного сервера
    """
    id: Optional[Any] = Field(..., description="Уникальный идентификатор дополнительной услуги для выделенного сервера.")
    price: Optional[Any] = Field(..., description="Стоимость (в рублях) дополнительной услуги для выделенного сервера.")
    ...
    def period_validate_enum(cls, value):
        ...
        if value not in ('P1D', 'P1M', 'P3M', 'P6M', 'P1Y', 'forever'):
            raise ValueError("must be one of enum values ('P1D', 'P1M', 'P3M', 'P6M', 'P1Y', 'forever')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DedicatedServerAdditionalService:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DedicatedServerAdditionalService:
        """Create an instance of DedicatedServerAdditionalService from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DedicatedServerAdditionalService.parse_obj(obj)

        _obj = DedicatedServerAdditionalService.parse_obj({
            "id": obj.get("id"),
            "price": obj.get("price"),
