...
class NodeOut(BaseModel):
    ...
    NodeOut
    """
    id: Optional[Any] = Field(..., description="Уникальный идентификатор ноды")
    created_at: Optional[Any] = Field(..., description="Дата и время создания ноды в формате ISO8601")
    type: Optional[Any] = Field(..., description="Тип ноды")
    group_id: Optional[Any] = Field(..., description="Идентификатор группы нод")
    status: Optional[Any] = Field(..., description="Статус")
    preset_id: Optional[Any] = Field(..., description="Идентификатор тарифа ноды")
    cpu: Optional[Any] = Field(..., description="Количество ядер")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NodeOut:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NodeOut:
        """Create an instance of NodeOut from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NodeOut.parse_obj(obj)

        _obj = NodeOut.parse_obj({
            "id": obj.get("id"),
