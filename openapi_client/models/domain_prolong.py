...
class DomainProlong(BaseModel):
    """
    Заявка на продление домена
    """
    action: Optional[Any] = Field(..., description="Тип создаваемой заявки.")
    fqdn: Optional[Any] = Field(..., description="Полное имя домена.")
    is_antispam_enabled: Optional[Any] = Field(None, description="Это логическое значение, которое показывает включена ли услуга \"Антиспам\" для домена")
    ...
    person_id: Optional[Any] = Field(None, description="Идентификатор администратора, на которого зарегистрирован домен.")
    prime: Optional[DomainPrimeType] = None
    __properties = ["action", "fqdn", "is_antispam_enabled", "is_autoprolong_enabled", "is_whois_privacy_enabled", "period", "person_id", "prime"]
    ...
    def action_validate_enum(cls, value):
        ...
        if value not in ('prolong'):
            raise ValueError("must be one of enum values ('prolong')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DomainProlong:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DomainProlong:
        """Create an instance of DomainProlong from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DomainProlong.parse_obj(obj)

        _obj = DomainProlong.parse_obj({
